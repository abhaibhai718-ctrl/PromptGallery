...
import os
import datetime
import html  # New import for safe text
...
def update_html(filename, prompt):
    # Sanitize prompt specifically for HTML attribute
    clean_prompt = html.escape(prompt)
    
    # New Card Template using data-prompt attribute (Safer)
    new_card = f"""
    ...
    # Check if HTML needs to be recreated (e.g. if user deleted it)
    ...
    # Insert new card at the TOP of the grid (Recent first)
...
<head>
    ...
    <style>
        ...
        .overlay {
            ...
            color: white; font-size: 13px; text-align: center; padding: 15px 5px 8px 5px;
        ...
        #toast {
            ...
            color: #fff; text-align: center; border-radius: 50px; padding: 12px 20px;
            position: fixed; z-index: 100; bottom: 40px; left: 50%;
    ...
    <script>
        async function copyPrompt(card) {
            ...
            try {
                ...
                // Method 2: Fallback for older browsers/insecure contexts
...
# Ensure HTML exists on first run
if not os.path.exists(HTML_FILE):
    create_base_html()
